...
def updateCommand(command,body,year,days,center,key=None):
        ...
        if center == '1': # moon
            command = re.sub('g,', 'g, -pc1', command)    
        if center == '2': # mercury
            command = re.sub('g,', 'g, -pc2', command) 
        if center == '3': # venus
            command = re.sub('g,', 'g, -pc3', command) 
        if center == '4': # mars
            command = re.sub('g,', 'g, -pc4', command) 
        if center == '5': # jupiter
            command = re.sub('g,', 'g, -pc5', command) 
        if center == '6': # saturn
            command = re.sub('g,', 'g, -pc6', command) 
        if center == '7': # uranus
            command = re.sub('g,', 'g, -pc7', command) 
        if center == '8': # neptune
            command = re.sub('g,', 'g, -pc8', command) 
        if center == '9': # pluto
            command = re.sub('g,', 'g, -pc9', command)             
        if key:
